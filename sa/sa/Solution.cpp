  ...
  costs(instance.getCosts())
{
    ...
    for(unsigned i = 0; i < numberOfTasks; i++) {
        ...
        for(unsigned i = 0; i < numberOfAgents; i++) {
            if(limits[minIndex] >= aux[minIndex]) { //se o recurso pra essa tarefa cabe no limite do agente
                limits[minIndex] -= aux[minIndex];
                this->tasks[i] = minIndex; //esse agente vira o dessa task
                break;
            }
            else {
                if (aux.size() > 1) { //se ainda tem agentes disponíveis,
                    aux.erase(aux.begin() + minIndex); //tira o elemento min do array, que nao tem limite suficiente
                    minIndex = getMinIndex(aux); //indice do novo valor mínimo
                    continue; //tenta atribuir de novo
                }
                else {
                    if (aux.size() == 1) {
                        this->tasks[i] = aux[0];
                        ...
                        int randoIndex = std::rand() % numberOfAgents;
                        this->tasks[i] = randoIndex; //se não tem mais agentes com limite disponível, bota um random e deixa ser unfeasible
                    }
                    break;
                }
            }
